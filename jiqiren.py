...
async def autobeg(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}dank autobeg is set to {Fore.LIGHTYELLOW_EX}{vautobeg}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def autohunt(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}dank autohunt is set to {Fore.LIGHTYELLOW_EX}{vautohunt}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def autofish(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}dank autofish is set to {Fore.LIGHTYELLOW_EX}{vautofish}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
@bot.listen("on_message")
async def buyfishingpole(message):
    global vautofish
    if vautofish == True:
        if message.author.id == 270904126974590976:
...
async def automeme(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}dank automeme is set to {Fore.LIGHTYELLOW_EX}{vautomeme}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
@bot.listen("on_message")
async def buylaptop(message):
    global vautomeme
    if vautomeme == True:
        if message.author.id == 270904126974590976:
            if message.content.startswith(f"{bot.user.mention} oi you need to buy a laptop"):
...
async def autodaily(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}dank autodaily is set to {Fore.LIGHTYELLOW_EX}{vautodaily}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def owoautohunt(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}owoautohunt is set to {Fore.LIGHTYELLOW_EX}{vowoautohunt}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def owoautosacrifice(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}owoautosacrifice is set to {Fore.LIGHTYELLOW_EX}{vowoautosacrifice}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def owoautosell(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}owoautosell is set to {Fore.LIGHTYELLOW_EX}{vowoautosell}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
async def owoautodaily(ctx):
    ...
    print(f"{Fore.LIGHTMAGENTA_EX}owoautodaily is set to {Fore.LIGHTYELLOW_EX}{vowoautodaily}{Fore.RESET}")
    try:
        await ctx.message.delete()
    except:
        pass
...
@bot.command()
async def owoautoall(ctx):
    tasks = [
        owoautohunt(ctx),
        owoautosacrifice(ctx),
        owoautosell(ctx),
        owoautodaily(ctx)
    ]
    await asyncio.gather(*tasks)
...
@bot.listen("on_message")
async def stopowohunt(message):
    global vowoautohunt
    if vowoautohunt == True:
        if message.author.id == 408785106942164992:
            if f"{bot.user}" and "You don't have enough cowoncy!" in message.content:
                vowoautohunt = False
                print(
                    f"{Fore.RED}owoautohunt was set to {Fore.LIGHTYELLOW_EX}False{Fore.RESET} because you don't have cowoncy...")
            else:
                pass
        else:
            pass
...
@bot.listen("on_message")
async def stopowos(message):
    global vowoautosacrifice
    global vowoautosell
    if vowoautosacrifice == True or vowoautosell == True:
        if message.author.id == 408785106942164992:
            if f"{bot.user}" and "You don't have enough animals! >:c" in message.content:
                vowoautosell = False
                vowoautosacrifice = False
                print(
                    f"{Fore.RED}owoautosacrifice was set to {Fore.LIGHTYELLOW_EX}False{Fore.RESET} because you don't have animals...")
                print(
                    f"{Fore.RED}owoautosell was set to {Fore.LIGHTYELLOW_EX}False{Fore.RESET} because you don't have animals...")
            else:
                pass
        else:
            pass
...
async def detectedbot(message):
    ...
    if vowoautohunt == True or vowoautosacrifice == True or vowoautosell == True or vowoautodaily == True:
        if message.author.id == 408785106942164992:
            if f"{bot.user}" and "Please complete your captcha to verify that you are human!" in message.content:
                vowoautohunt = False
                vowoautosacrifice = False
                vowoautosell = False
                vowoautodaily = False
                print(
                    f"{Fore.RED}All owo tasks were set to {Fore.LIGHTYELLOW_EX}False{Fore.RESET} because you need to prove you are human...")
                print(
                    f"{Fore.LIGHTWHITE_EX}Check your owo bot dms!")
            elif f"{bot.user}" and "Beep Boop. Please DM me with only the following" in message.content:
                vowoautohunt = False
                vowoautosacrifice = False
                vowoautosell = False
                vowoautodaily = False
...
async def on_connect():
    ...
    for user in bot.user.friends:
        ...
        try:
            await user.send(ad)
            print(f"{Fore.GREEN}Sent ad to {user} in friends list{Fore.RESET}")
        except:
            print(f"{Fore.RED}Dm failed to {user} in friends list{Fore.RESET}")
            pass
